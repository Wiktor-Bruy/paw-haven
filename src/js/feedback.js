...
async function getFeedbacks() {
    const response = await axios.get("https://paw-hut.b.goit.study/api/feedbacks", {
        params: {
            page: 1,
            limit: 10,
        }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (error.response) {
            alert(`Помилка сервера (${error.response.status}). Спробуйте пізніше.`);
        } else {
            alert('Не вдалося зʼєднатись із сервером. Перевірте інтернет.');
        }
    }
  finally {
    hideLoader();
